...
// Global para o label do contador
...
void mostrarCarouselSlave(std::vector<Receita> lista, int indice) {
  ...
  lv_obj_set_style_bg_color(cursor_obj, lv_palette_main(LV_PALETTE_BLUE),
                            0); // BLUE aparece RED
  ...
  // CORRIGIDO: Amarelo Ouro (RGB: FF D7 00). No BGR pode precisar inverter RB
  // se driver nao fizer. O usuário disse que "Logo MSA é Amarelo"
  ...
  if (lista.empty()) {
    ...
    lv_obj_set_style_text_color(lblVazio, lv_palette_main(LV_PALETTE_RED),
                                0); // RED aparece BLUE (Aviso)
  } else {
...
void mostrarTelaProducao(Receita r) {
  ...
  lv_obj_add_event_cb(btnProximo, event_handler_proximo_producao,
...
void mostrarAlertaCaixaCheia() {
  // Cria um container modal (cobre tudo)
  lv_obj_t *modal = lv_obj_create(lv_scr_act());
  lv_obj_set_size(modal, 220, 180);
  lv_obj_center(modal);
  lv_obj_set_style_bg_color(modal, lv_palette_main(LV_PALETTE_GREEN), 0);
  lv_obj_set_style_radius(modal, 15, 0);
  lv_obj_set_style_border_width(modal, 4, 0);
  lv_obj_set_style_border_color(modal, lv_color_white(), 0);

  // Texto
  lv_obj_t *lbl = lv_label_create(modal);
  lv_label_set_text(lbl, "META ATINGIDA\n\nCAIXA CHEIA");
  lv_obj_set_style_text_align(lbl, LV_TEXT_ALIGN_CENTER, 0);
  lv_obj_set_style_text_font(lbl, &lv_font_montserrat_22, 0);
  lv_obj_set_style_text_color(lbl, lv_color_white(), 0);
  lv_obj_center(lbl);

  // Opcional: Botão para fechar, mas como o sistema trava aqui...
  // Vamos deixar apenas o aviso visual.
}
